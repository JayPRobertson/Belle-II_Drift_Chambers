...
G4double HelixApproach::TimeAtCylinderRadius(G4double radius, G4double halfLength) const {
    ...
    */
    ...
    do {
        ...
        if (std::abs(deriv) < 1e-12){
             G4cout << "ERROR: Derivative too small. Current deriv = " 
                   << std::abs(deriv) << G4endl;
            return -1.0;
        }
    ...
    //if (std::abs(pos.z()) > halfLength){
         //G4cout << "ERROR: Length exceeded limit. Current zpos = " 
                   //<< std::abs(pos.z()) << ", halfLength = " << halfLength << G4endl;
            //return -1.0;
    //}
...
void HelixApproach::FindGasVolumeCrossings(
    ...
    G4double tEntry = TimeAtCylinderRadius(innerRadius, halfLength);
    G4double tExit = TimeAtCylinderRadius(outerRadius, halfLength);
